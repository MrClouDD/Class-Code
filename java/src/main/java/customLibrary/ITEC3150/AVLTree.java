...
public class AVLTree<E> extends BST<E> {
    ...
    @Override /** Override createNewNode to create an AVLNode */
    ...
    private void updateHeight(Node<E> node) {
        if (node.left == null && node.right == null) // node is a leaf
            ...
            node.height = 1 + ((Node<E>) (node.right)).height;
        else if (node.right == null) // node has no right subtree
            node.height = 1 + ((Node<E>) (node.left)).height;
        else
            node.height = 1 +
                    Math.max(((Node<E>) (node.right)).height,
                            ((Node<E>) (node.left)).height);
    }
    ...
    private void balancePath(E e) {
        ...
        for (int i = path.size() - 1; i >= 0; i--) {
            ...
            switch (balanceFactor(A)) {
                case -2:
                    if (balanceFactor((Node<E>) A.left) <= 0) {
                    ...
                    break;
                case +2:
                    if (balanceFactor((Node<E>) A.right) >= 0) {
    ...
    private int balanceFactor(Node<E> node) {
        if (node.right == null) // node has no right subtree
            ...
            return ((Node<E>) node.right).height -
                    ((Node<E>) node.left).height;
    }
    ...
    private void balanceLR(Node<E> A, Node<E> parentOfA) {
        Node<E> B = A.left; // A is left-heavy
        Node<E> C = B.right; // B is right-heavy
    ...
    private void balanceRL(Node<E> A, Node<E> parentOfA) {
        Node<E> B = A.right; // A is right-heavy
        Node<E> C = B.left; // B is left-heavy
